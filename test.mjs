/**
 * Game7 Anonymous Bricklayer - Gameplay Test
 * Tests: basic load, touch interaction, game state progression, no freeze/dead-end
 * Run: node test-game7.mjs [game-dir]
 */
import { chromium } from 'playwright';
import { resolve } from 'path';

const W = 375, H = 812;
const DIR = process.argv[2] || 'game7-anonymous-bricklayer';
...
async function test() {
  ...
  try {
    ...
    // 1. Game area exists (canvas or HTML game container)
    ...
    // 3. Interactive elements have adequate touch targets (>= 44px)
    ...
    // 4. No keyboard-dependent interaction (all primary actions accessible via touch)
    const keyboardDep = await page.evaluate(() => {
      // Check if keydown listeners are the ONLY input method
      var hasTouchStart = !!document.querySelector('[ontouchstart], .touch-zone, .tap-area');
      var hasClickBtns = document.querySelectorAll('button, .btn, [role="button"]').length > 0;
      var hasCanvas = !!document.querySelector('canvas');
      return { touchStart: hasTouchStart, buttons: hasClickBtns, canvas: hasCanvas };
    });
    (keyboardDep.buttons || keyboardDep.canvas)
      ? pass('Touch-accessible input methods found')
      : fail('No touch-accessible input found');

    // 5. Game state can progress — tap to start and verify something changes
    await page.evaluate(() => {
    ...
    stateChanged ? pass('Game content renders after interaction') : fail('No meaningful content after interaction');

    // 6. No horizontal overflow (mobile-friendly)
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
    overflow <= 2 ? pass('No horizontal overflow') : fail(`Overflow: ${overflow}px`);

    // 7. No console errors after interaction
    errors.length === 0 ? pass('No JS errors after interaction') : fail(`Errors after interaction: ${errors.join('; ')}`);
